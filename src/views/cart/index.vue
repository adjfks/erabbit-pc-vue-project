  ...
  <div class="xtx-cart-page">
    <div class="container">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-for="item in $store.getters['cart/validList']" :key="item.skuId">
              <td><XtxCheckbox :modelValue="item.selected" @change="checkOne(item.skuId, $event)" /></td>
              <td>
                <div class="goods">
              ...
              <td class="tc">
                <XtxNumbox :modelValue="item.count" @change="changeCount(item.skuId, $event)" />
              </td>
              <td class="tc">
          ...
          <tbody>
            ...
            <tr v-for="item in $store.getters['cart/invalidList']" :key="item.skuId">
              <td><XtxCheckbox style="color: #eee" :modelValue="item.selected" @change="checkOne(item.skuId, $event)" /></td>
              <td>
                <div class="goods">
                  <RouterLink :to="`/product/${item.id}`"><img :src="item.picture" alt="" /></RouterLink>
                  <div>
              ...
              <td class="tc">1</td>
              <td class="tc">
                <p>&yen;{{ (item.nowPrice * 100 * item.count) / 100 ? (item.nowPrice * 100 * item.count) / 100 : '' }}</p>
              </td>
              <td class="tc">
                <p><a class="green" href="javascript:;" @click="deleteCart(item.skuId)">删除</a></p>
                <p><a href="javascript:;">找相似</a></p>
      ...
      <div class="action">
        ...
        <div class="total">
          共 {{ $store.getters['cart/validTotal'] }} 件商品，已选择 {{ $store.getters['cart/validTotal'] }} 件，商品合计：
